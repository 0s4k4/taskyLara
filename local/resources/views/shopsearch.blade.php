...
<body>
	...
	<div class="video">
	<div class="headerbg">
	 <div class="col-md-12" align="center"><h1>{{ __('user.search') }}</h1></div>
	 </div>
	<div class="container">
   ...
   {!! csrf_field() !!}
	...
	<div class="col-md-3 swidth nocity">	
		...
		<input type="text"  name="city" id="txtPlaces" class="form-control" placeholder="{{ __('user.enter_city') }}" value="<?php if(!empty($city)){ echo $city; } ?>">
	</div>	
	
	
	<div class="col-md-3 swidth nocity">
	<select name="star_rate" class="form-control">
	<option value="">{{ __('user.star_rating') }}</option>
	<option value="1" <?php if(!empty($star_rate)) { if($star_rate==1){?> selected <?php } } ?>>{{ __('user.one_star') }}</option>
	<option value="2" <?php if(!empty($star_rate)) { if($star_rate==2){?> selected <?php } } ?>>{{ __('user.two_stars') }}</option>
	<option value="3" <?php if(!empty($star_rate)) { if($star_rate==3){?> selected <?php } } ?>>{{ __('user.three_stars') }}</option>
	<option value="4" <?php if(!empty($star_rate)) { if($star_rate==4){?> selected <?php } } ?>>{{ __('user.four_stars') }}</option>
	<option value="5" <?php if(!empty($star_rate)) { if($star_rate==5){?> selected <?php } } ?>>{{ __('user.five_stars') }}</option>
	</select>
	</div>
	...
	<div class="row">
		...
		<?php foreach($newsearches as $shop){  	?>
				...
				<?php 
						...
						<img align="center" class="sthumb" src="<?php echo $url.$npaths;?>" alt="{{ __('user.profile_hoto') }}"/>
						<?php } else { ?>
				...
				<div align="center"><a href="<?php echo $url; ?>/vendor/<?php echo $shop->name;?>" class="btn btn-success radiusoff">{{ __('user.view_shop_book') }}</a></div>
			</div> 
...
function createMarker(results) {
	...
	{{ $newsearches->appends(\Request::except('_token'))->render() }}
