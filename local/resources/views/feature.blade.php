...
<body>
	...
	<div class="video">
	<div class="headerbg">
	 <div class="col-md-12" align="center"><h1>{{ __('user.feature_job') }}</h1></div>
	 </div>
	...
	<div class="container">
	 ...
	 <?php if(!empty($gig_cnt)){
      ...
      if($gig[0]->job_type=="request")
	  {
		   ...
		   $vtxt =  __('user.feature_request_para1');
		   $htxt =  __('user.feature_request');
	  }
	  else
	  {
		  $urwel = $url.'/service/'.$gig[0]->gid;
		  $vtxt =  __('user.feature_service_para1');
		  $htxt =  __('user.feature_service');
	  }
...
<?php /* ?> Duration: <?php echo $site_setting[0]->featured_days;?> days <?php */?>
	...
	<form action="<?php echo $paypal_url; ?>" method="post">
		...
		<input type="submit" name="submit" value="{{ __('user.pay_using_paypal') }}" class="btn sv_paypal">
...
$hash = strtolower(hash('sha512', $hash_string));
    ...
    <input type="submit" name="submit" value="{{ __('user.pay_using_payumoney') }}" class="btn sv_payu">
</form>
</div>
    <?php 
	...
	<a href="<?php echo $url;?>/feature/buyer/<?php echo Auth::user()->id;?>/<?php echo $gig[0]->gid;?>" class="btn btn-primary">{{ __('user.pay_available_balance') }}</a>
	<?php } ?>
