...
<body>
	...
	<div class="headerbg">
	 <div class="col-md-12" align="center"><h1>{{ __('user.job_details') }}</h1></div>
	 </div>
	...
	<?php if($sql_request[0]->featured==1){?><div class="new_ribbon"><span>{{ __('user.featured') }}</span></div><?php } ?>
	  ...
	  <?php 
		...
		<p class="uppercase fleft font15">{{ __('user.created_c') }}<span class=""> <?php echo $newDate;?></span>, in <?php echo $catname;?><?php if(!empty($namesub)){?>, <?php echo $namesub;?><?php } ?></p>
	  <span class="clearfix"></span>
    ...
    <span class="share"></span>
	  ...
	  <h3 class="fleft violet_text"><span class="fontsize21">{{ __('user.budget') }} </span> <?php echo $estim;?></h3>
	...
	<div>
      ...
      <p class="sv_project col-md-3"><strong>{{ __('user.skills') }}:</strong></p><p class="col-md-9"> <?php echo $topar;?></p>
      <div style="clear:both;"></div>	 
	 
	  <p class="sv_project col-md-3"><strong>{{ __('user.category') }}:</strong></p><p class="col-md-9"> <?php echo $catname;?><?php if(!empty($namesub)){?>,<?php echo $namesub;?><?php } ?></p>
	 <div style="clear:both;"></div>
	  <?php if(!empty($sql_request[0]->preferred_location)){?>
	
	  <p class="col-sm-3 sv_project"><strong>{{ __('user.preferred_location') }}:</strong> </p><p class="col-sm-9"> <?php echo $sql_request[0]->preferred_location;?></p>
	  <?php } ?>
 <div style="clear:both;"></div>
	  <p class="col-sm-3 sv_project"><strong>{{ __('user.file_attachment') }}:</strong> </p><p class="col-sm-9"> 
	  <?php if (Auth::guest()) {?>
	...
	<?php echo $secondfile;?>
	  ...
	  <div style="clear:both;"></div>
	  <p class="col-sm-3 sv_project"><strong>{{ __('user.estimate_delivery') }}:</strong></p><p class="col-sm-9"> <?php if(!empty($sql_request[0]->complete_days)){?> <?php echo $sql_request[0]->complete_days;?> <?php echo $day_txt;?> <?php } ?></p>
	 <div style="clear:both;"></div>
	 
	  <p class="col-sm-3 sv_project"><strong>{{ __('user.status') }}:</strong></p><p class="col-sm-9">  <span style="color:<?php echo $r_clr;?>;"><?php echo $r_status;?></span></p>
	  ...
	  <h3>{{ __('user.offer_work_job_bid') }}</h3>
	  
	  <div class="row ">
        <div class="col-md-4">
		    <label>Y{{ __('user.bid_job') }}</label>
            <input type="number" placeholder="Price" class="form-control testBox">
         </div>
        <div class="col-md-4">
		<label>{{ __('user.your_email_address') }}</label>
            <input type="email" placeholder="Email" class="form-control testBox">
        </div>
        <div class="col-md-4">
		<label></label>
            <a href="javascript:void(0);" class="btn btn-success btn-block bid_btn" onclick="alert('login user only');">{{ __('user.submit') }}</a>
        </div>
	</div>
	...
	<div class="height40"></div>
	  ...
	  <div class="row">
        <div class="col-md-6">
		    <label>{{ __('user.bid_job') }} (<?php echo $site_setting[0]->site_currency;?>)</label>
		...
		<div class="col-md-12">
		<label>{{ __('user.describe_your_proposal') }}</label>
		...
		<div class="col-md-12">
		<label>{{ __('user.estimated_time_complete') }}</label>
	...
	<div class="height40"></div>
	  ...
	  {*/
		 ...
		 ?>
		 <p class="ash_border"></p>
		 <h3><?php echo $total_bids;?> {{ __('user.freelancers_bidding_job') }}</h3>
        <a name="gotonew"></a>
	...
	if(!empty($userr[0]->photo)){?>
		...
		<p class="black"><strong>{{ __('user.estimate') }}</strong> : <?php if(!empty($bid->proposal_estimate)) {?><?php echo $bid->proposal_estimate;?> <?php echo $txtt_days;?> <?php } ?></p>
		<?php if($bid->award==1){?>
		<span class="btn award_apply">{{ __('user.awarded') }}</span>
		<?php } 
	...
	<?php if (Auth::guest()) {?>
	<a href="javascript:void(0);" class="btn bit_apply btn-large" onclick="alert('Login user only');"> {{ __('user.apply_for_job') }}</a>
	...
	<a href="<?php echo $url;?>/request/<?php echo $sql_request[0]->gid;?>/<?php echo $sql_request[0]->request_slug;?>" class="btn bit_apply btn-large">{{ __('user.awarded_you') }}</a>
	
	<?php } else if($award_check[0]->prop_user_id!=Auth::user()->id) { ?>
	
	<a href="<?php echo $url;?>/request/<?php echo $sql_request[0]->gid;?>/<?php echo $sql_request[0]->request_slug;?>#goto" class="btn bit_apply btn-large">{{ __('user.awarded_other') }}</a>
	...
	<a href="<?php echo $url;?>/request/<?php echo $sql_request[0]->gid;?>/<?php echo $sql_request[0]->request_slug;?>#goto" class="btn bit_apply btn-large">{{ __('user.apply_for_job') }}</a>
	<?php } ?>
	...
	<a href="<?php echo $url;?>/request/<?php echo $sql_request[0]->gid;?>/<?php echo $sql_request[0]->request_slug;?>#gotonew" class="btn award_apply btn-large">{{ __('user.awarded') }}</a>
	<?php } else {?>
	...
	<span class="font18">{{ __('user.min_bid_price') }}</span>
	...
	<span class="font18">{{ __('user.max_bid_price') }}</span>
	</div>
	...
	<span class="font18">{{ __('user.total_bids') }}</span>
	</div>
