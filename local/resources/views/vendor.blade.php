...
<body>
	...
	<?php 
					   $shopheader="/shop/";
						...
						?>
        <img align="left" class="fb-image-lg" src="<?php echo $url.$path;?>" alt="{{ __('user.cover_banner') }}"/>
						<?php } else { ?>
						<img align="left" class="fb-image-lg" src="<?php echo $url.'/local/images/no-image-big.jpg';?>" alt="{{ __('user.cover_banner') }}"/>
						<?php } ?>
		<div>
		<?php if($shop[0]->featured=="yes"){?><div class="ribbon_new"><span>{{ __('user.featured') }}</span></div><?php } ?>
		<?php $shopphoto="/shop/";
						$paths ='/local/images'.$shopphoto.$shop[0]->profile_photo;
						if($shop[0]->profile_photo!=""){?>
        <img align="left" class="fb-image-profile thumbnail" src="<?php echo $url.$paths;?>" alt="{{ __('user.profile_hoto') }}"/>
						<?php } else { ?>
						<img align="left" class="fb-image-profile thumbnail customwidth" src="<?php echo $url.'/local/images/nophoto.jpg';?>" alt="{{ __('user.profile_hoto') }}"/>
						<?php } ?>
			</div>			
		...
		<a href="<?php echo $url;?>/send-message/<?php echo Auth::user()->id;?>/<?php echo $shop[0]->user_id;?>" class="btn btn-primary btn-md radiusoff" style="margin-top:-5px;">{{ __('user.send_message') }}</a>
		<?php } } else { ?>
		<a href="javascript:void(0);" class="btn btn-primary btn-md radiusoff" onclick="alert('login user only');" style="margin-top:-5px;">{{ __('user.send_message') }}</a>
		<?php } ?>
	...
	<li class="active"><a href="#sent" data-toggle="tab"><span class="lnr lnr-cog blok"></span>{{ __('user.services') }} </a></li>
        <li><a href="#inbox" data-toggle="tab"><span class="lnr lnr-user blok"></span> {{ __('user.profile') }}</a></li>
      
      <li><a href="#assignment" data-toggle="tab"><span class="lnr lnr-star blok"></span> {{ __('user.reviews') }} </a></li>
	  
	  <li><a href="#gallery" data-toggle="tab"><span class="lnr lnr-picture blok"></span> {{ __('user.gallery') }} </a></li>
	  
	   <li><a href="#contact" data-toggle="tab"><span class="lnr lnr-phone-handset blok"></span> {{ __('user.contact_vendor') }} </a></li>
	...
	<div class="tab-pane active" id="sent">
			...
			<img src="<?php echo $url.$path;?>" border="0" class="img-responsive imgradius" alt="">
				...
				<!-- se comenta la linea de precios en la carta de servicios -->
					...
					<!-- <div style="clear: both;"></div> -->
								<button type="button" class="vendor-description " data-toggle="modal" data-target="#description_{{$sellerservice->id}}">
								  {{ __('user.description') }}
								</button>		
					
				</div>
					<div class="modal fade" id="description_{{$sellerservice->id}}" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
						<div class="modal-dialog" role="document">
							<div class="modal-content">
								<div class="modal-header">
									...
									<h4 class="modal-title" id="modalLabel">{{$sellerservice->subname}}</h4>
		...
		<?php if(Auth::user()->id!=$shop[0]->user_id){?>
					...
					<input type="submit" name="register-submit" id="register-submit" tabindex="4" class="booknow radiusoff" value="{{ __('user.book_now') }}"></a>
				    </div>
				<?php } } else { ?>
					<div class="col-md-12">
					<a href="<?php echo $url;?>/booking/<?php echo $vendor;?>/<?php echo $sellerservice->subid;?>/<?php echo $userid;?>" class="radiusoff">
	...
	<div class="tab-pane" id="inbox">
	 ...
	 <div class="col-md-12">
		...
		<div class="form-group">
                            <label for="name" class="col-md-12">{{ __('user.your_name') }}  <span class="require">*</span></label>
		...
		<div class="form-group">
                            <label for="name" class="col-md-12">{{ __('user.your_email') }} <span class="require">*</span></label>
		...
		<div class="form-group">
                            <label for="phone_no" class="col-md-12">{{ __('user.your_phone_no') }} <span class="require">*</span></label>
		...
		<div class="form-group">
                            <label for="name" class="col-md-12">{{ __('user.your_message') }} <span class="require">*</span></label>
