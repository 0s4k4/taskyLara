...
<body>
	...
	<div class="headerbg">
	 <div class="col-md-12" align="center"><h1>{{ __('user.my_posted_job') }}</h1></div>
	 </div>
	<div class="container">
	...
	<div class="table-responsive">
	  ...
	  <table id="mytable" class="table table-striped table-bordered table-hover datatable">
                   ...
                   <th> {{ __('user.date') }}</th>
                    <th>{{ __('user.job_name') }}</th>
                     ...
                     <th>{{ __('user.delivery') }}</th>
                       ...
                       <th>{{ __('user.estimated') }}</th>
					   <th>{{ __('user.action') }}</th>
					   
					   <th>{{ __('user.feature_job_c') }} </th>
					   
					   <th>{{ __('user.message_freelancer') }} </th>
					   
					   <th>{{ __('user.fund_status') }}</th>
					   </tr>
                   </thead>
			...
			<?php echo $offer_count;?> {{ __('user.bids') }}
			</a>
			
			<?php if($request->request_status==0){?>
			<a href="<?php echo $url;?>/my_request/delete/<?php echo $request->gid;?>" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete?');">{{ __('user.delete') }}</a>
			<?php } ?>
	...
	<span class="btn btn-warning" style="cursor:default; border-radius:none;">{{ __('user.featured_progress') }}</a>
	<?php } else if($request->featured==1 && $request->status==1){ ?>
	<span class="btn btn-success" style="cursor:default; border-radius:none;">{{ __('user.featured_c') }}</a>
	<?php } else if($request->featured==0 && $request->status==1){?>
	
	<a href="<?php echo $url;?>/feature/<?php echo $request->gid;?>" class="btn btn-primary">{{ __('user.make_feature') }}</a></td>
    <?php } else { ?>
	<?php } ?>
			</td>
