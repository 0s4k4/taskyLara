...
<?php $url = URL::to("/"); ?>
	...
	<div class="headerbg">
	 <div class="col-md-12" align="center"><h1>{{ __('user.my_bookings') }}</h1></div>
	 </div>
	...
	<?php 
			...
			{
			  ?>
			  <div class="err-msg" align="center">{{ __('user.no_booking_found') }}</div>
			<?php
			} else {
				$iq=1;
...
$(function(){
  ...
  $('.starrr').on('starrr:change', function(e, value){
    ratingsField.val(value);
			...
			<div class="row booking_page">
				...
				<h3 class="sv_shop_style"><a href="<?php echo $url; ?>/vendor/<?php echo $book->name;?>" target="_blank"><?php echo $book->shop_name; ?></a></h3>
					...
					<h5>{{ __('user.booking_id') }} <?php echo $book->book_id; ?> <br/><br/>
					
					{{ __('user.payment_method') }}<?php echo $book->payment_mode; ?> <br/><br/>
    	...
    	<div class="well-sm mtop10">
            <div class="revbtn">
                <a class="btn btn-success btn-green radiusoff" href="#reviews-anchor" id="open-review-box<?php echo $bookk; ?>">{{ __('user.leave_review') }}</a> 
				
				<a class="btn btn-danger" href="<?php echo $url;?>/invoice/<?php echo base64_encode($book->book_id);?> ">{{ __('user.invoice') }}</a>
				<a href="<?php echo $url;?>/send-message/<?php echo Auth::user()->id;?>/<?php echo $book->user_id;?>" class="btn btn-primary btn-md radiusoff">{{ __('user.send_message') }}</a>
				...
				<a href="#" class="btn btn-danger radiusoff" data-toggle="modal" data-target="#myDispute<?php echo $book->book_id;?>">{{ __('user.dispute') }}</a>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title"><?php echo $book->shop_name; ?> - {{ __('user.dispute') }}</h4>
        </div>
    ...
    <label for="email">{{ __('user.booking_id') }}  <?php echo $book->book_id;?></label>
  ...
  <button type="submit" class="btn btn-success">{{ __('user.submit') }}</button>
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('user.close') }}</button>
			...
			<a href="<?php echo $url;?>/my_bookings/cancel/<?php echo base64_encode($book->book_id);?>" onclick="return confirm('Are you sure you want to do this?');">{{ __('user.cancel_booking') }}</a>
			</div>
				<?php } } ?>
			
			<div style="margin-top:10px;">
			
			<div class="total-price radiusoff">{{ __('user.total_price') }} <?php if($book->total_amt=="") { echo "0"; } else { echo $book->total_amt; }?>&nbsp;<?php echo $book->currency; ?></div>
				
				<div style="clear:both; height:10px;"></div>
			
				<div>
				<strong>{{ __('user.payment_status') }}</strong> <?php echo $view_list[0]->status; ?><?php if(!empty($view_list[0]->reject)){ echo ','.$view_list[0]->reject; } ?>
				</div>
				<div style="clear:both; height:10px;"></div>
				
				<div>
				<?php if(empty($view_list[0]->reject)){?>
				<strong>{{ __('user.service_status') }}</strong> <?php if($book->service_complete==0){?>Awaiting completion<?php } ?> <?php if($book->service_complete==1){?>Completed<?php } ?> <?php if($book->service_complete==2){?>Released the payment<?php } ?>
            ...
            <div class="row" id="post-review-box<?php echo $bookk; ?>" style="display:none;">
                <div class="col-md-12">
					...
					<input type="hidden" name="rate_id" value="<?php if(!empty($nucount)) { echo $viewrating[0]->rid; } ?>">
                        ...
                        <textarea required class="form-control animated radiusoff" cols="50" id="new-review<?php echo $shop_id; ?>" name="comment" placeholder="{{ __('user.enter_your_review_here') }}" rows="5"><?php if(!empty($nucount)){ echo $viewrating[0]->comment; } ?></textarea>
                        ...
                        <div class="text-right">
							...
							<?php if(config('global.demosite')=="yes"){?><button type="button" class="btn btn-success btn-md radiusoff btndisable">{{ __('user.save') }}</button> 
								<span class="disabletxt">( <?php echo config('global.demotxt');?> )</span><?php } else { ?>
							
                            <button class="btn btn-success btn-md radiusoff" type="submit">{{ __('user.save') }}</button>
								<?php } ?>
