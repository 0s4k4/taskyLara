...
<body>
	...
	<div class="video">
	<div class="headerbg">
	 <div class="col-md-12" align="center"><h1>{{ __('user.services') }}</h1></div>
	 </div>
	<div class="container">
	...
	<form class="form-horizontal" role="form" method="POST" action="{{ route('services') }}" id="formID" enctype="multipart/form-data">
   		...
   		<div class="row">
			...
			<div class="col-sm-4">
				<div class="col-sm-12">
					<div class="form-group" >
						<label>{{ __('user.services_name') }}<span class="star">*</span></label>
						<select class="form-control input-lg validate[required]" id="subservice_id" name="subservice_id" required>
							<option value="">{{ __('user.select_services') }}</option>
			...
			<div class="col-sm-2">
				<div class="col-sm-12">
					<div class="form-group">		
						<label>{{ __('user.price') }} <span class="star">*</span></label>
			...
			<div class="col-sm-4">
				<div class="col-sm-12">
					<div class="form-group" id="shop_address" >
						<label>{{ __('user.time_hours') }}</label>
		...
		<div class="row">
			<div class="col-sm-12">		                      
				<a href="<?php echo $url;?>/services" class="btn btn-primary radiusoff">{{ __('user.reset') }}</a>
				<?php if(config('global.demosite')=="yes"){?>
				<button type="button" class="btn btn-success btn-md radiusoff btndisable">{{ __('user.submit') }}</button> 
	...
	<div class="row" align="right" style="margin-bottom:2px;">
	 <?php if(config('global.demosite')=="yes"){?><span class="disabletxt">( <?php echo config('global.demotxt');?> ) </span><button type="button" class="btn btn-primary radiusoff btndisable">{{ __('user.add_services') }}</button> 
								<?php } else { ?>
	
	 <a href="<?php echo $url;?>/services" class="btn btn-primary radiusoff">{{ __('user.add_services') }}</a>
								<?php } ?>
  ...
  foreach($viewservice as $newserve){?>
    <tr>
	  ...
	  <?php } else { ?>
	  <a href="<?php echo $url;?>/services/<?php echo $newserve->id;?>"><img src="<?php echo $url.'/local/images/edit.png';?>" alt="{{ __('user.edit') }}" border="0"></a>
	  <?php } ?>
	  ...
	  <img src="<?php echo $url.'/local/images/delete.png';?>" alt="{{ __('user.delete') }}" border="0"></a></td>
	  <?php } ?>
    </tr>
  <?php $ii++; } ?>
