...
<?php $url = URL::to("/"); ?>
	...
	<div class="headerbg">
	 <div class="col-md-12" align="center"><h1><?php echo e(__('user.my_bookings')); ?></h1></div>
	 </div>
	...
	<?php 
			...
			{
			  ?>
			  <div class="err-msg" align="center"><?php echo e(__('user.no_booking_found')); ?></div>
			<?php
			} else {
				$iq=1;
...
$(function(){
    ...
    ratingsField.val(value);
			...
			<div class="row booking_page">
				...
				<h3 class="sv_shop_style"><a href="<?php echo $url; ?>/vendor/<?php echo $book->name;?>" target="_blank"><?php echo $book->shop_name; ?></a></h3>
					...
					<h5><?php echo e(__('user.booking_id')); ?> <?php echo $book->book_id; ?> <br/><br/>
					
					<?php echo e(__('user.payment_method')); ?><?php echo $book->payment_mode; ?> <br/><br/>
    	...
    	<div class="well-sm mtop10">
            <div class="revbtn">
                <a class="btn btn-success btn-green radiusoff" href="#reviews-anchor" id="open-review-box<?php echo $bookk; ?>"><?php echo e(__('user.leave_review')); ?></a> 
				
				<a class="btn btn-danger" href="<?php echo $url;?>/invoice/<?php echo base64_encode($book->book_id);?> "><?php echo e(__('user.invoice')); ?></a>
				<a href="<?php echo $url;?>/send-message/<?php echo Auth::user()->id;?>/<?php echo $book->user_id;?>" class="btn btn-primary btn-md radiusoff"><?php echo e(__('user.send_message')); ?></a>
				...
				<a href="#" class="btn btn-danger radiusoff" data-toggle="modal" data-target="#myDispute<?php echo $book->book_id;?>"><?php echo e(__('user.dispute')); ?></a>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title"><?php echo $book->shop_name; ?> - <?php echo e(__('user.dispute')); ?></h4>
        </div>
        <div class="modal-body">
          
		  <form class="" role="form" method="POST" action="<?php echo e(route('my_book')); ?>" id="formID" enctype="multipart/form-data">
          <?php echo e(csrf_field()); ?>

		  <div class="form-group">
    <label for="email"><?php echo e(__('user.booking_id')); ?>  <?php echo $book->book_id;?></label>
  ...
  <div class="form-group">
    <label for="email"><?php echo e(__('user.subject')); ?></label>
    <input type="text" class="form-control validate[required]" id="subject" name="subject">
  </div>
  <div class="form-group">
    <label for="pwd"><?php echo e(__('user.message')); ?></label>
    <textarea class="form-control validate[required]" name="message" style="min-height:100px;"></textarea>
  </div>
  
  <button type="submit" class="btn btn-success"><?php echo e(__('user.submit')); ?></button>
</form>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-default" data-dismiss="modal"><?php echo e(__('user.close')); ?></button>
        </div>
      </div>
			...
			<a href="<?php echo $url;?>/my_bookings/cancel/<?php echo base64_encode($book->book_id);?>" onclick="return confirm('Are you sure you want to do this?');"><?php echo e(__('user.cancel_booking')); ?></a>
			</div>
				<?php } } ?>
			
			<div style="margin-top:10px;">
			
			<div class="total-price radiusoff"><?php echo e(__('user.total_price')); ?> <?php if($book->total_amt=="") { echo "0"; } else { echo $book->total_amt; }?>&nbsp;<?php echo $book->currency; ?></div>
				
				<div style="clear:both; height:10px;"></div>
			
				<div>
				<strong><?php echo e(__('user.payment_status')); ?></strong> <?php echo $view_list[0]->status; ?><?php if(!empty($view_list[0]->reject)){ echo ','.$view_list[0]->reject; } ?>
				</div>
				<div style="clear:both; height:10px;"></div>
				
				<div>
				<?php if(empty($view_list[0]->reject)){?>
				<strong><?php echo e(__('user.service_status')); ?></strong> <?php if($book->service_complete==0){?>Awaiting completion<?php } ?> <?php if($book->service_complete==1){?>Completed<?php } ?> <?php if($book->service_complete==2){?>Released the payment<?php } ?>
				...
				if(!empty($dispwives))
				{
					...
					{ 
				       $viewstatus = "<strong>".__('user.dispute_status') ."</strong>". __('user.awaiting_admin_action') ;
					}
                    else
					{
						$viewstatus = "<strong>". __('user.dispute_status') ."</strong>".$disp_view[0]->status;
					}					
            ...
            <div class="row" id="post-review-box<?php echo $bookk; ?>" style="display:none;">
                <div class="col-md-12">
					...
					<input type="hidden" name="rate_id" value="<?php if(!empty($nucount)) { echo $viewrating[0]->rid; } ?>">
                        ...
                        <textarea required class="form-control animated radiusoff" cols="50" id="new-review<?php echo $shop_id; ?>" name="comment" placeholder="<?php echo e(__('user.enter_your_review_here')); ?>" rows="5"><?php if(!empty($nucount)){ echo $viewrating[0]->comment; } ?></textarea>
                        ...
                        <div class="text-right">
                            ...
                            <?php echo e(__('user.cancel')); ?></a>
							
							
							<?php if(config('global.demosite')=="yes"){?><button type="button" class="btn btn-success btn-md radiusoff btndisable"><?php echo e(__('user.save')); ?></button> 
								<span class="disabletxt">( <?php echo config('global.demotxt');?> )</span><?php } else { ?>
							
                            <button class="btn btn-success btn-md radiusoff" type="submit"><?php echo e(__('user.save')); ?></button>
								<?php } ?>
...
<?php /**PATH C:\xampp\htdocs\venkat\tasky\tasky\local\resources\views/my_bookings.blade.php ENDPATH**/ ?>