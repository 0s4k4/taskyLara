...
class MessageController extends Controller
{
	...
	public function my_chat_history($sender)
	{
		...
		$view_message = DB::table('tbl_private_message')
			...
			$type = $selected_user[0]->admin;
		
		$data = array('view_message' => $view_message, 'view_message_count' => $view_message_count, 'sender' => $sender, 'receiver' => $receiver, 'usernamo' => $usernamo,'type' => $type);
   ...
   public function single_message(Request $request)
   {
	   ...
	   $view_count = DB::table('tbl_private_message')
		...
		$user_chk2 = DB::table('users')
                        ->where('id', '=', $receiver)
						->get();
	   if($user_chk2[0]->admin==2)
	   {
		   $types = "seller";
	   }
	   else if($user_chk2[0]->admin==0)
	   {
		   $types = "customer";
	   }
	   else if($user_chk2[0]->admin==1)
	   {
		   $types = "admin";
	   }
		if(empty($view_count))
		{
			DB::insert('insert into tbl_private_message (sender,receiver,message,send_by,submitted) values (?, ?,?, ?,?)', [$receiver,$sender,'',$types,$date_submitted]);
		}	
   ...
   public function post_message(Request $request)
   {
	...
	DB::insert('insert into tbl_private_message (sender,receiver,message,send_by,read_status,submitted) values (?, ?,?, ?, ?,?)', [$sender,$receiver,$message_txt,$send_by,$read_status,$date_submitted]);
